from fastapi import APIRouter, Depends, status, Form, Path
...
@router.post("/create", summary="Create a new book", status_code=status.HTTP_201_CREATED)
...
@router.get("/get/{book_id}", summary="Get a book by ID", response_model=BookCreate, status_code=status.HTTP_200_OK)
def get_book(session: Annotated[Session, Depends(get_db)], book_id: Annotated[str, Path(description="The ID of the book to retrieve")]):
    """
...
@router.patch('/update/{book_id}', summary="Update an existing book", status_code=status.HTTP_200_OK)
...
@router.delete('/delete/{book_id}', summary="Delete a book by ID", status_code=status.HTTP_204_NO_CONTENT)
